<script lang="ts">
	...
	const currentPrompt = $derived(
		'visitor@PortfoliOS:' + '~/portfolio' + terminal.currentPath + '$ '
	);
...
<div id="terminalWrapper" class="sticky right-0 bottom-0">
	...
	{:else}
		<div
			class={`fixed ${!terminal.isLineExpanded ? 'pointer-events-none translate-y-4 opacity-0' : 'pointer-events-auto translate-y-0 opacity-100'} bottom-10 z-50 w-full border-t border-t-gray-700 bg-gray-200 px-5 pt-2 font-mono transition-all duration-300 dark:bg-[#0c0c0c] dark:text-[#e4e4e4]`}
			transition:fly={{ y: 16, duration: 220 }}
		>
			{#if latest == undefined}
				Welcome to PortfoliOS! Type 'help' to start.
			{:else}
				{latest.path}{latest.command}
				<br />
	...
	>
		...
		<button
			...
			class="justify-center px-5 py-0 font-[NerdFont] text-xl font-bold">󰄿</button
		>
		<button
			onclick={toggleTerminal}
			class="justify-center border-l border-l-gray-400 py-0 pr-3 pl-5 font-[NerdFont] text-lg"
			></button
		>
	</form>
	<button
		onclick={toggleTerminal}
		class={`${terminal.isLineExpanded ? 'hidden' : ''} absolute right-0 bottom-0 z-100 h-17 w-17 rounded-t-2xl rounded-bl-2xl border-t-2 border-l-2 border-gray-500 bg-gray-200 p-3.5 font-[NerdFont] text-3xl shadow-none shadow-gray-700 transition-all hover:shadow-xl dark:bg-[#0c0c0c] dark:text-[#e4e4e4] dark:shadow-gray-400`}
		></button
	>
</div>
